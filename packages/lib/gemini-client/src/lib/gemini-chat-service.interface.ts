...
export interface IGeminiChatService {
  ...
  /**
   * 프롬프트를 전송하고 응답 완료까지 대기한다.
   *
   * 내부적으로 텍스트 입력 → 전송 버튼 클릭 → 응답 완료 폴링을 수행한다.
   * 응답이 완료되면 `ModelResponse`를 반환한다.
   *
   ...
   * @param options - 타임아웃, 취소 등 옵션
   */
  generate(prompt: string, options?: GenerateOptions): Promise<Result<ModelResponse, GeminiError>>;
